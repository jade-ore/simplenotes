        ...
        <>
            <div id="create-note" className="bg-gray-100 w-fit mx-auto block px-6 mt-3 rounded py-3 outline-2">
                <form onSubmit={handleNewNote} className="flex flex-col items-center">
                    ...
                    <button className="bg-slate-600 active:bg-slate-600 hover:bg-slate-500 text-white font-bold p-2 rounded outline-2" type="submit">Make Note (will not save)</button>
                </form>
            </div>

            <div className="grid">
                {notes ? notes.map((note) => (
                    <NoteDemo key={note.id} deleteSelf={deleteNote} title={note.title} desc={note.description} note_id={note.id}></NoteDemo>
                )) : <></>}
            </div>
        </>
    )
